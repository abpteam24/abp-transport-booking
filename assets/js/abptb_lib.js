...
function abptb_init(target = abptb_parent) {
    ...
    abptb_load_image(target);
    abptb_toast_init();
    abptb_required(target);
    abptb_load_datepicker(target);
    abptb_init_dynamic_date_pickers();
...
function abptb_load_more(parent = abptb_parent) {
...
function abptb_load_image(parent = abptb_parent) {
...
function abptb_load_datepicker(parent = abptb_parent) {
    ...
    parent.find(".abp_datepicker:visible").each(function () {
        ...
        parent.find(".abp_datepicker:visible").datepicker({
            dateFormat: abptb_var.date_format, autoSize: true, changeMonth: true, changeYear: true, //showButtonPanel: true,
...
function abptb_init_dynamic_date_pickers(newSelector = null, newConfig = null) {
    window.abptb_picker_data = window.abptb_picker_data || {};
    if (newSelector && newConfig) {
        window.abptb_picker_data[newSelector] = newConfig;
    }
    if (jQuery.isEmptyObject(window.abptb_picker_data)) {
        return;
    }
    let currentDateFormat = (typeof abptb_var !== 'undefined' && abptb_var.date_format) ? abptb_var.date_format : 'yy-mm-dd';
    jQuery.each(window.abptb_picker_data, function (selector, config) {
...
function abptb_toast_init() {
    if (abptb_parent.find('.toast_msg_area').length === 0) {
        abptb_parent.first().append('<div class="toast_msg_area"></div>');
    }
    let toast_notices = abptb_parent.find('.toast_notice');
    if (toast_notices.length > 0) {
        toast_notices.each(function (index) {
            ...
            setTimeout(function () {
                abptb_toast_msg(msg, type);
            }, index * 600);
        });
    }
}
function abptb_required(parent = abptb_parent) {
...
//==============================================================================================//
function abptb_alert($this, attr = 'alert') {
    alert($this.data(attr));
}
function abptb_page_scroll(target) {
    jQuery('html, body').animate({
        scrollTop: target.offset().top -= 150
    }, 1000);
}
function abptb_toast_msg(msg, type = 'info') {
    const icons = {success: '✅', error: '❌', warn: '⚠️', info: 'ℹ️'};
    const el = jQuery(`<div class="toast_msg_box ${type}"><span>${icons[type] || 'ℹ️'}</span><span>${msg}</span></div>`);
    abptb_parent.find('.toast_msg_area').append(el).hide().fadeIn(200);
    setTimeout(() => el.fadeOut(300, () => el.remove()), 3400);
}
function abptb_wc_price_format(price) {
    if (typeof price === 'string') {
        price = Number(price);
    }
    price = price.toFixed(abptb_var.decimal_num);
    let total_part = price.toString().split(".");
    total_part[0] = total_part[0].replace(/\B(?=(\d{3})+(?!\d))/g, abptb_var.thousands_separator);
    price = total_part.join(abptb_var.currency_decimal);
    let price_text = '';
    if (abptb_var.currency_position === 'right') {
        price_text = price + abptb_var.currency_symbol;
    } else if (abptb_var.currency_position === 'right_space') {
        price_text = price + '&nbsp;' + abptb_var.currency_symbol;
    } else if (abptb_var.currency_position === 'left') {
        price_text = abptb_var.currency_symbol + price;
    } else {
        price_text = abptb_var.currency_symbol + '&nbsp;' + price;
    }
    if (abptb_var.currency_suffix) {
        price_text = price + '&nbsp;' + abptb_var.currency_suffix;
    }
    return price_text;
}
...
function abptb_data_change($this) {
    abptb_load_image();
...
function abptb_icon_change(currentTarget) {
...
function abptb_text_change(currentTarget) {
...
function abptb_class_change(currentTarget) {
...
function abptb_input_value_change(currentTarget) {
...
document.body.addEventListener('click', function (event) {
    const targetDiv = event.target.closest('div.abptb_area [data-href]');
    if (targetDiv) {
...
window.abptb_date_close = function (element, e) {
...
function abptb_target_close(close_id, $this) {
...
function abptb_target_open(close_id, $this) {
    if ($this.closest('.tf_close_area').length > 0) {
...
function abptb_popup_close(target_id = '') {
    if (target_id) {
...
(function ($) {
    ...
    function get_collapse_target(current, id = '') {
        ...
        if (current.closest('.data_single_collapse').length > 0) {
            ...
            return abptb_parent.find('[data-collapse="' + target_id + '"]');
        }
    }
            ...
            $(parent_target_close).find('[data-collapse]').each(function () {
                if ($(this).hasClass('abp_active')) {
                    ...
                    if (collapse_id !== currentTarget.data('collapse-target')) {
                        ...
                        abptb_text_change(target_collapse);
                        abptb_icon_change(target_collapse);
                    }
                }
            })
        }
...
(function ($) {
    "use strict";
    //==============================================================================Qty inc dec================//
    $(document).on("click", "div.abptb_area .qty_decrease ,div.abptb_area .qty_increase", function () {
        let current = $(this);
        ...
        if (!$this.hasClass('abp_active')) {
            ...
            parent.find('.abp_active[data-radio]').each(function () {
                if ($(this).attr('data-close-target')) {
                    ...
                    abptb_target_close(close_id, $this);
                }
                ...
                if ($this.attr('data-close-target')) {
                    let close_id = $this.attr('data-close-target');
                    abptb_target_open(close_id, $this);
                }
    ...
    //==============================================================================custom select ================//
    $(document).on("click", "div.abptb_area .abp_dropdown .dropdown_list li", function (e) {
        e.preventDefault();
        let current = $(this);
    ...
    }, 'div.abptb_area .abp_dropdown input[type="text"]');
}(jQuery));
...
function abptb_live_pagination(parent) {
    let btn = parent.find('.live_pagination');
    ...
    abptb_pagination_item(parent);
    if (typeof abptb_load_image === "function") {
        abptb_load_image();
    }
}
function abptb_pagination_item(parent) {
    let is_filter_active = parent.find('.pagination_item.abp_on, .pagination_item.abp_off').length > 0;
    let hidden_items;
    let total_items;
    if (!is_filter_active) {
...
(function ($) {
    ...
    $(document).on('click', 'div.abptb_area .grid_view', function () {
        ...
        if (container) {
            ...
            setTimeout(function () {
                container.removeClass('abptb_lists').addClass('abptb_grid');
    ...
    $(document).on('click', 'div.abptb_area .list_view', function () {
        ...
        if (container) {
            container.addClass('view-switching');
            setTimeout(function () {
                container.removeClass('abptb_grid').addClass('abptb_lists');
                container.removeClass('view-switching');
                abptb_load_more();
            }, 150);
        }
...
document.addEventListener('DOMContentLoaded', () => {
    document.querySelectorAll('div.abptb_area [data-slider]').forEach(el => new ABPTBSlider(el));
    const gallery_item = document.querySelectorAll('div.abptb_area .gallery_item');
    ...
    const faq_item = document.querySelectorAll('div.abptb_area .faq_item');
