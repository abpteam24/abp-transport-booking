<?php
    ...
    if (!class_exists('ABPTF_Ticket')) {
        class ABPTF_Ticket {
            public function __construct() {
                add_action('abptf_post_content', [$this, 'ticket_configuration']);
                add_action('wp_ajax_abptf_type_switch', [$this, 'type_switch']);
            }
            public function ticket_configuration($post_infos = []): void {
                ...
                <div class="tab_item abptf_ticket" data-tabs="#abptf_ticket">
                    <div class="group_setting">
                            ...
                            <label class="_fj_between">
                                ...
                                <?php if (ABPTF_Function::on_off('sp')) { ?>
                                    <select class="_form_control " name="seat_type" required>
                                        ...
                                        <option value="sp" <?php echo esc_attr($seat_type == 'sp' ? 'selected' : ''); ?>><?php echo esc_html(ABPTF_Layout::ticket_type('sp')); ?></option>
                                        <option value="ticket" <?php echo esc_attr($seat_type == 'ticket' ? 'selected' : ''); ?>><?php echo esc_html(ABPTF_Layout::ticket_type('ticket')); ?></option>
                                    </select>
                                <?php } else { ?>
                        ...
                        <?php if (ABPTF_Function::on_off('min_max')) { ?>
                            <div class="setting_item">
                                <label class="_fj_between _f_wrap">
                                    <span class="_abp_label"><?php esc_html_e('Min Qty/Order', 'abp-transportforge'); ?></span>
                                    <input class="_form_control validation_number" name="min_qty" value="<?php echo esc_attr($post_infos['min_qty'] ?? 0); ?>" placeholder="<?php esc_attr_e('Ex: 1', 'abp-transportforge'); ?>"/>
                                </label>
                                <div class="_divider_xxs"></div>
                                <?php ABPTF_Layout::info_text('min_qty'); ?>
                            </div>
                            <div class="setting_item">
                                <label class="_fj_between _f_wrap">
                                    <span class="_abp_label"><?php esc_html_e('Max Qty/Order', 'abp-transportforge'); ?></span>
                                    <input class="_form_control validation_number" name="max_qty" value="<?php echo esc_attr($post_infos['max_qty'] ?? ''); ?>" placeholder="<?php esc_attr_e('Ex: 5', 'abp-transportforge'); ?>"/>
                                </label>
                                <div class="_divider_xxs"></div>
                                <?php ABPTF_Layout::info_text('max_qty'); ?>
                            </div>
                        <?php } ?>
            ...
            public function type_switch(): void {
                ...
                $html = ob_get_clean();
                wp_send_json_success(['html' => $html, 'msg' => __('Ticket Type Switched Successfully...!', 'abp-transportforge'), 'type' => 'success']);
            }
            public function seat_type($post_infos): void {
                $sp_infos = $post_infos['sp_infos'] ?? [];
                $display_ticket_type = $post_infos['display_ticket_type'] ?? 'on';
                $display_ticket_type = ABPTF_Function::on_off('ticket_type') ? $display_ticket_type : 'off';
                $sp_data = ABPTF_Query::get_sp();
                $post_id = $post_infos['post_id'] ?? '';
                //echo '<pre>';print_r(ABPTF_sp_ticket);echo '</pre>';
                ?>
                <div class="_fj_between">
                    ...
                    <button type="button" class="_btn_light_active_xs">
                        ...
                        <span class="_abp_color_theme _mar_l_xs transport_total"><?php echo esc_html(ABPTF_Function::get_total_qty($post_id, $post_infos)); ?></span>
                    </button>
                    <div class="_group_content">
                        <?php ABPTF_Layout::button_add(__('Add New Seat Label', 'abp-wc-transport-manager')); ?>
                    </div>
                </div>
                ...
                <div class="_ov_auto">
                    <table class="_abp">
                        ...
                        <tr>
                            ...
                            <?php if (ABPTF_Function::on_off('ticket_type') && $display_ticket_type == 'on') { ?>
            ...
            public function ticket_type($post_infos): void {
                ...
                <div class="_fj_between">
                    ...
                    <div class="_group_content">
                        <?php if (ABPTF_Function::on_off('ticket_type') && $display_ticket_type == 'on') {
                            ABPTF_Layout::button_add(__('Add New item', 'abp-wc-transport-manager'));
                        }
            ...
            public function ticket_item($field = array(), $id = ''): void {
                ...
                <tr class="delete_area">
                    <td>
                        <label>
                            <select name="ticket_name[]" class='_form_control' data-required>
                                ...
                                <?php if (!empty(ABPTF_ticket)) {
                                    foreach (ABPTF_ticket as $key => $ticket) { ?>
                    ...
                    <td>
                        <label>
                            <input data-required type="number" pattern="[0-9]*" step="1" class="_form_control validation_number" name="ticket_qty[]" placeholder="<?php esc_attr_e('EX: 15', 'abp-transportforge'); ?>" value="<?php echo esc_attr($field['qty'] ?? 10); ?>"/>
                        </label>
                    </td>
                    <td>
                        <label>
                            <input type="number" pattern="[0-9]*" step="1" class="_form_control validation_number" name="reserve_qty[]" placeholder="<?php esc_attr_e('EX: 15', 'abp-transportforge'); ?>" value="<?php echo esc_attr($field['reserve'] ?? 0); ?>"/>
                        </label>
                    </td>
                    <td>
                        <label>
                            <input type="number" pattern="[0-9]*" step="1" class="_form_control validation_number" name="ticket_min_qty[]" placeholder="<?php esc_attr_e('EX: 1', 'abp-transportforge'); ?>" value="<?php echo esc_attr($field['min_qty'] ?? 1); ?>"/>
                        </label>
                    </td>
                    <td>
                        <label>
                            <input type="number" pattern="[0-9]*" step="1" class="_form_control validation_number" name="ticket_max_qty[]" placeholder="<?php esc_attr_e('EX: 15', 'abp-transportforge'); ?>" value="<?php echo esc_attr($field['max_qty'] ?? ''); ?>"/>
                        </label>
                    </td>
                    <td>
                        <label>
                            <textarea class="_form_control" name="ticket_description[]" placeholder="<?php esc_attr_e('EX: Description', 'abp-transportforge'); ?>"><?php echo esc_html($field['description'] ?? ''); ?></textarea>
                        </label>
                    </td>
            ...
            public function sp_item($sp_data, $display_ticket_type, $field = array()): void {
                ...
                <tr class="delete_area">
                    ...
                    <td>
                        <label>
                            <input type="text" class="_form_control validation_name" name="sp_name[]" placeholder="<?php esc_attr_e('EX: Lower Deck', 'abp-transportforge'); ?>" value="<?php echo esc_attr($field['name'] ?? ''); ?>"/>
                        </label>
                    </td>
                    <?php if (ABPTF_Function::on_off('ticket_type') && $display_ticket_type == 'on') { ?>
                        <th class="ticket_type_details">
                            <?php ABPTF_Seat_Plan::sp_seat_list(ABPTF_ticket, $meta_info); ?>
                        </th>
                    <?php } ?>
                    ...
                    <td>
                        <div class="_all_center">
                            <div class="_group_content">
                                <button type="button" class="_btn_light_theme_xxs sp_id_change" onclick="abptf_popup_open_global('view_sp','<?php echo esc_attr($id); ?>')" title="<?php esc_attr_e('View : Seat Plan', 'abp-transportforge'); ?>">👁️</button>
                                <?php
