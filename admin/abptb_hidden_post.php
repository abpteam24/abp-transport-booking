<?php
	...
	if ( ! class_exists( 'ABPTB_Hidden_Post' ) ) {
		class ABPTB_Hidden_Post {
			...
			public function insert_wc_hidden_post( $post_id, $post ): void {
				...
				if ( $post->post_type === $cpt && $post->post_status === 'publish' ) {
					if ( empty( ABPTB_Function::get_post_info( $post_id, 'exit_wc_hidden_post' ) ) ) {
			...
			public function save_hidden_post( $post_id ): void {
				...
				if ( $this->count_hidden_post( $post_id ) === 0 || empty( ABPTB_Function::get_post_info( $post_id, 'link_wc_id' ) ) ) {
					$this->create_wc_hidden_post( $post_id, $title );
				}
				$product_id = ABPTB_Function::get_post_info( $post_id, 'link_wc_id', $post_id );
				if ( ! $product_id ) {
					return;
				}
			...
			public function hide_hidden_post_frontend(): void {
				...
				if ( is_array( $visibility ) && ! empty( $visibility ) ) {
					if ( $visibility[0]->slug === 'exclude-from-catalog' || $visibility[0]->name === 'exclude-from-catalog' ) {
						$check_event_hidden = (int) ABPTB_Function::get_post_info( $post->ID, 'abptb_link_id', 0 );
			...
			public function create_wc_hidden_post( $post_id, $title ): void {
				...
				$product->update_meta_data( 'abptb_link_id', $post_id );
				$pid = $product->save();
				if ( $pid ) {
					wp_set_object_terms( $pid, (int) $post_id, 'abptb_link_taxonomy' );
					...
					wp_cache_delete( 'abptb_count_hidden_post_' . $post_id, 'abptb_counts' );
					delete_transient( 'abptb_count_hidden_post_' . $post_id );
				}
			}
			public function count_hidden_post( $post_id ): int {
				$cache_group = 'abptb_counts';
				$cache_key   = 'abptb_count_hidden_post_' . $post_id;
				...
				if ( false === $count ) {
					...
					if ( false === $count ) {
						...
						$count = (int) $wpdb->get_var( $wpdb->prepare(
							"SELECT COUNT(*) FROM {$wpdb->postmeta} WHERE meta_key = 'abptb_link_id' AND meta_value = %s",
							(string) $post_id
						) );
			...
			public function exclude_url_from_search_engine(): void {
				...
				if ( ! empty( $visibility ) && is_object( $visibility[0] ) ) {
					if ( $visibility[0]->slug === 'exclude-from-catalog' || $visibility[0]->name === 'exclude-from-catalog' ) {
						$check_hidden = (int) ABPTB_Function::get_post_info( $post->ID, 'abptb_link_id', 0 );
						if ( $check_hidden > 0 ) {
			...
			public function get_all_hidden_product_id(): array {
				$product_ids = [];
				$query       = ABPTB_Query::query_post_type( ABPTB_Function::get_cpt() );
				if ( isset( $query->posts ) && is_array( $query->posts ) ) {
					foreach ( $query->posts as $result ) {
						$wc_id = ABPTB_Function::get_post_info( $result->ID, 'link_wc_id' );
						if ( $wc_id ) {
