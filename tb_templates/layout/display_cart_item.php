<?php
	...
	add_action( 'abptb_display_cart_item_template', function ( $cart_item = [] ) {
		...
		if ( ! empty( $ticket_infos ) && is_array( $ticket_infos ) ) {
            ...
            <div class="abptb_area">
                <div class="_section_xs">
                    <h6 class="_abp _color_theme"><?php esc_html_e( 'Booking Information : ', 'abp-transport-booking' ); ?></h6>
                    <div class="_divider_xxs"></div>
                    <ul class="_abp cart_list">
                        <li>
                            <span class="fas fa-calendar-check _mar_r_xs"></span>
                            <span class="_fs_label"><?php esc_html_e( 'Rent Start : ', 'abp-transport-booking' ); ?></span>&nbsp;<?php echo esc_html( ABPTB_Function::date_format( $start_time ) ); ?>
                        </li>
                        <li>
                            <span class="fas fa-calendar-check _mar_r_xs"></span>
                            <span class="_fs_label"><?php esc_html_e( 'Rent End : ', 'abp-transport-booking' ); ?></span>&nbsp;<?php echo esc_html( ABPTB_Function::date_format( $end_time ) ); ?>
                        </li>
                        <li>
                            <span class="fas fa-business-time _mar_r_xs"></span>
                            <span class="_fs_label"><?php esc_html_e( 'Duration : ', 'abp-transport-booking' ); ?></span>&nbsp;<?php echo esc_html( $duration ); ?>
                        </li>
						<?php if (  ! empty( $location ) ) { ?>
                            <li>
                                <span class="fas fa-location _mar_r_xs"></span>
                                <span class="_fs_label"><?php esc_html_e( 'Location : ', 'abp-transport-booking' ); ?></span>&nbsp;<?php echo esc_html( ABPTB_Function::location_value( $location ) ); ?>
                            </li>
						<?php } ?>
                    </ul>
                </div>
                <div class="_section_xs">
                    <h6 class="_abp _color_theme"><?php esc_html_e( 'Property Information : ', 'abp-transport-booking' ); ?></h6>
					<?php
						foreach ( $ticket_infos as $ticket_info ) {
                            ...
                            <div class="_divider_xxs"></div>
                            <ul class="_abp cart_list">
                                <li><span class="_fs_label"><?php esc_html_e( 'Name : ', 'abp-transport-booking' ); ?></span><?php echo esc_html( $ticket_name ); ?></li>
                                <li><span class="_fs_label"><?php esc_html_e( 'Quantity : ', 'abp-transport-booking' ); ?></span><?php echo esc_html( $ticket_qty ); ?></li>
                                <li><span class="_fs_label"><?php esc_html_e( 'Rent : ', 'abp-transport-booking' ); ?></span><?php echo wp_kses_post( $price_html ); ?></li>
								<?php if ( ABPTB_Function::on_off( 'deposit' ) && ! empty( $deposit ) ) { ?>
                                    <li><span class="_fs_label"><?php esc_html_e( 'Deposit : ', 'abp-transport-booking' ); ?></span><?php echo wp_kses_post( wc_price( $deposit ) ); ?></li>
								<?php } ?>
								<?php if ( ! empty( $brand ) && ABPTB_Function::on_off( 'brand' ) ) { ?>
                                    <li><span class="_fs_label"><?php echo esc_html( ABPTB_Function::brand_label() ); ?></span><?php echo esc_html( ABPTB_Function::brand_value( $brand ) ); ?></li>
								<?php } ?>
                            </ul>
						<?php } ?>
                </div>
