<?php
	...
	add_action( 'abptb_post_filter_template', function ( $params ) {
		...
		if ( empty( $loc_id ) && is_array( $post_ids ) && ! empty( $post_ids ) ) {
			foreach ( $post_ids as $post_id ) {
				$location_array = ABPTB_Function::get_post_info( $post_id, 'abptb_location' );
		...
		if ( $cat_count > 1 || $loc_count > 1 || $style === 'grid' || $style === 'list' ) {
            ...
            <div class="post_top_filter">
				<?php
					...
					if ( $loc_count > 1 ) {
						if ( $loc_count > 4 && is_array( ABPTB_Location ) ) {
							?>
                            <label>
                                <select class="_form_control" name="loc_id">
                                    <option value="" selected><?php esc_html_e( 'All Location', 'abp-transport-booking' ); ?></option>
									<?php
                            ...
                            <div class="custom_radio _group_content">
                                ...
                                <div class="radio_item">
                                    <button type="button" class="_btn_light_info_xs_fs_h6 abp_active" data-radio="" data-open-icon="fa-check-circle" data-close-icon="fa-circle">
                                        <span data-icon class="_mar_r_xs far fa-check-circle"></span><?php esc_html_e( 'All Location', 'abp-transport-booking' ); ?>
                                    </button>
                                </div>
								<?php
									foreach ( $locations as $current_loc_id ) {
