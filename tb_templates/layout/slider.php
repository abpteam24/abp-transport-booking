<?php
	...
	add_action( 'abptb_slider_template', function ( $img_ids = [], $abptb_slider = [] ) {
        if ( empty( $img_ids ) || ! is_array( $img_ids ) ) {
            return;
        }
		$total_images        = count( $img_ids );
		$slider_indicator    = ( $abptb_slider['indicator_visible'] ?? 'on' ) ?: 'on';
		$indication_position = ( $abptb_slider['indication_position'] ?? 'bottom' ) ?: 'bottom';
		$indication_position = ( $slider_indicator === 'on' ) ? $indication_position : '';
		$popup_id            = uniqid( '#abptb_slider_' );
		$active_popup        = ( $abptb_slider['visible_popup'] ?? 'on' ) ?: 'on';
		?>
        <div class="abptb_slider">
